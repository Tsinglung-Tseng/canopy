...
状态：已实现（src/llm/（kernel/openai/mock/agents/provider）+ src/search.ts，2026-06-11；自带 kernel 2026-06-12）

## 职责

Canopy 所有 LLM 调用的唯一出口，基于自带最小 kernel 的原语组合（ADR-005/ADR-008）。三个 agent：
...
```
src/llm/
  kernel.ts      // 最小编排内核：Cost/Outcome/Budget/ask/askSchema/par/run（ADR-008）
  openai.ts      // 零依赖 OpenAI-compatible 后端（全局 fetch，schema 三模式）
  mock.ts        // MockLlm：确定性免 key 测试后端
...
零外部依赖（ADR-008：kernel + OpenAI-compatible 后端内联，全局 fetch 无 SDK）。Plexus 为可选增强——kernel 的 `Llm` 接口是 Plexus `Llm` 的结构子集，Plexus 后端实例可直接注入，无需 import。模型/凭据从 corpus 配置注入，**无任何硬编码默认**（缺失即崩，CLAUDE.md 纪律）。
